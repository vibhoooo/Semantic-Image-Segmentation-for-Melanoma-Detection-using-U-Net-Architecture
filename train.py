...
def train(model, train_loader, opt, criterion, epoch):
    ...
    with open('logs.txt', 'a') as file:
        for batch_idx, sample_batched in enumerate(train_loader):
            ...
            output = model(data)
            m = nn.Sigmoid()
            # print("BEFORE ceil ", output.requires_grad)
            # print("MAX VALUE OF TENSOR: ", torch.max(output))
            # print("OUTPUT IS:\n", output)
            ...
            # print("After ceil ", output.requires_grad)
...
def run_test(model, test_loader, opt):
    ...
    :return:
        - predictions: list, for each element, numpy array (Width, Height)
        - img_ids: list, for each element, an image id string
    """
    model.eval()
    predictions = []
    img_ids = []
