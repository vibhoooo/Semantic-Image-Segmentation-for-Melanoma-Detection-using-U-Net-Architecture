...
def show_batch(sample_batched):
    ...
    images_batch, masks_batch = sample_batched['image'].numpy().astype(np.uint8), sample_batched['mask'].numpy().astype(
        np.bool)
...
class DSB2018Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.train:
            ...
            mask = io.imread(mask_dir, as_gray=True).astype(np.bool_)
            mask = np.expand_dims(mask, axis=-1)
            sample = {'image': img, 'mask': mask, 'img_id': self.img_id[idx], "height": img.shape[0],
                      "width": img.shape[1]}
